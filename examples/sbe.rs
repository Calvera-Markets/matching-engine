//! SBE order entry and SBE public tape.
//!
...
fn parse_args() -> Config {
    let mut cfg = Config {
        ...
        md_ip: Ipv4Addr::new(239, 0, 0, 1),
        md_port: 5000,
        md_iface: Ipv4Addr::LOCALHOST,
        cpu_ingress: 8,
        cpu_engine: 10,
        cpu_egress: 12,
        cpu_md: 14,
    ...
    while let Some(a) = args.next() {
        ...
        match a.as_str() {
            ...
            "--md-ip" => cfg.md_ip = take(&mut args).parse().unwrap(),
            "--md-port" => cfg.md_port = take(&mut args).parse().unwrap(),
            "--md-iface" => cfg.md_iface = take(&mut args).parse().unwrap(),
            ...
            "-m" | "--cpu-md" => cfg.cpu_md = take(&mut args).parse().unwrap(),
            "--slab" => cfg.slab = take(&mut args).parse().unwrap(),
            "--symbol" => cfg.symbol = take(&mut args),
            "-h" | "--help" => {
                eprintln!(
                    "sbe [--port 12345] [--wal orderbook.wal] [--symbol AAPL]\n\
                     [--md-ip 239.0.0.1] [--md-port 5000] [--md-iface 127.0.0.1]\n\
                     [--cpu-ingress 8] [--cpu-engine 10] [--cpu-egress 12] [--cpu-md 14]\n\
                     [--slab 1048576]"
                );
                std::process::exit(0);
            }
...
fn main() {
    ...
    let oe = SbeOe::new(&cfg.symbol);
    let mut ingress = Ingress::bind(cfg.port, cmds.clone(), oe.clone()).unwrap_or_else(|e| {
        eprintln!("bind :{}: {e}", cfg.port);
        std::process::exit(1);
    });
    let mut egress = Egress::new(private, oe);
    let mut md = MdPub::new(
        public,
        cfg.md_ip,
        cfg.md_port,
        cfg.md_iface,
        SbeMd::new(cfg.symbol.as_bytes()),
    )
    .unwrap_or_else(|e| {
        eprintln!("md socket: {e}");
        std::process::exit(1);
    });
    ...
    eprintln!(
        "matching-engine live  sbe {} :{}  md={}:{}",
        cfg.symbol, cfg.port, cfg.md_ip, cfg.md_port
    );
